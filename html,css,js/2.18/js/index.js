window.addEventListener("load", function () {
  ...
  let ul = focus.children[0]; // 获取focus的第一个孩子，也就是ul
  let ol = focus.children[1]; //获取focus的第二个孩子，也就是ol
  let w = focus.offsetWidth; // 获取focus的宽度
  let index = 0; // 用来记录图片索引
  let timer = setInterval(function () {
    // 添加定时器，两秒调用一次
    index++; // 每调用一次（轮播一次），图片索引号+1
    let translatex = -index * w; // ul要移动的距离
  ...
  ul.addEventListener("transitionend", function () {
    ...
    // 让底部小圆点跟着一起动
    // 将带有current类的li去掉该类 remove
    ol.querySelector(".current").classList.remove("current");
    // 给当前li添加current类 add
    ol.children[index].classList.add("current");
  });
  // 手指滑动轮播图
  let startX = 0; // 用来存储手指初始位置
  let moveX = 0; // 用来存储手指在屏幕上移动的距离
  let flag = false; // 记录用户是否在图上移动了手指
  // 给ul绑定手指触摸事件
  ...
  ul.addEventListener("touchend", function (e) {
    if (flag) {
      // flag==true（移动图片）时才去判断用户的移动方向和距离
